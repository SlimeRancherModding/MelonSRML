...
namespace MelonSRML.Console
{
	...
	{
		// BINDING FILE
		internal static string bindFile = Path.Combine(Application.persistentDataPath, "console.bindings");
		...
		public static void RegisterBind(string id, string text, string command)
		{
			if (id.Equals("all"))
			{
				MelonLogger.Warning($"Trying to register user defined button with id 'all' but 'all' is not a valid id!");
				return;
			}
		...
		public static bool RemoveBind(string id)
		{
			...
			MelonSRML.Console.Console.cmdButtons.Remove("user." + id);
