...
namespace MelonSRML.Console
{
	...
	public abstract class ConsoleCommand
	{
        ...
        internal MelonBase belongingMod;
		/// <summary>
		...
		protected virtual bool ArgsOutOfBounds(int argCount, int min = -1, int max = -1)
		{
			...
			{
				MelonLogger.Error($"The '{ID}' command got less arguments then expected (Had: {argCount}; Min: {min})");
				return true;
			}

			if (argCount > max && max > -1)
			{
				MelonLogger.Error($"The '{ID}' command got more arguments then expected (Had: {argCount}; Max: {max})");
				return true;
			}
