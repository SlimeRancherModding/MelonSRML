...
namespace MelonSRML.Console.Commands
{
	...
	public class HelpCommand : ConsoleCommand
	{
		...
		public override bool Execute(string[] args)
		{
			...
			if (args == null)
			{
				MelonLogger.Msg("<color=cyan>List of Commands Available:</color>");
				MelonLogger.Msg("<i><> is a required argument; [] is an optional argument</i>");
			...
			else
			{
				if (Console.commands.ContainsKey(args[0]))
				{
					...
					{
						MelonLogger.Msg($"<color=#77DDFF>{cmd.Usage}</color> - {cmd.Description}");
						foreach (string line in cmd.ExtendedDescription.Split('\n'))
							MelonLogger.Msg(line);
					}
					else
					{
						MelonLogger.Warning($"No extended description was found for command '<color=#77DDFF>{cmd.ID}</color>'. Showing default description");
						MelonLogger.Msg($"<color=#77DDFF>{Console.ColorUsage(cmd.Usage)}</color> - {cmd.Description}");
					}
				}
				else
				{
					MelonLogger.Error($"Command '<color=white>{args[0]}</color>' not found");
				}
			}
