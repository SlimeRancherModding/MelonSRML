...
namespace MelonSRML.Console
{
    ...
    public class ConsoleWindow : MonoBehaviour
    {
        ...
        private static readonly string openMelonLoaderLog = "MelonLoader Log";
        ...
        private static float CursorX => GUIUtility.GetStateObject(Il2CppType.Of<TextEditor>(), GUIUtility.keyboardControl).Cast<TextEditor>().graphicalCursorPos.x;
        ...
        {
            GameObject window = new GameObject("_ConsoleWindow", Il2CppType.Of<ConsoleWindow>());
            DontDestroyOnLoad(window);
            cursorlockHandler = window.AddComponent<CursorLockHandler>();
            cursorlockHandler.enabled = false;
            window.hideFlags |= HideFlags.HideAndDontSave;

            MelonLogger.Msg("Attached Console Window successfully!");
        }

        // TO ENSURE IT GOT CREATED
        private void Start()
        {
            MelonLogger.Msg("Console Window running.");
            MelonLogger.Msg("Use command '<color=#77DDFF>help</color>' for a list of all commands");
            MelonLogger.Msg("Use command '<color=#77DDFF>mods</color>' for a list of all mods loaded");
            MelonLogger.Msg("You can also check the menu on the right");

            foreach (MelonInfoAttribute info in MelonBase.RegisteredMelons.Select(x => x.Info))
            {
                modsText += $"{(modsText.Equals(string.Empty) ? "" : "\n")}<color=#77DDFF>{info.Name}</color> [<color=#77DDFF>Author:</color> {info.Author} | <color=#77DDFF>Version:</color> {info.Version}]";
            }
        }
        ...
        private void Update()
        {
            foreach (KeyBindManager.ConsoleBind bind in KeyBindManager.ModBindingsV01.consoleBinds)
            {
                if (bind.action.WasPressedThisFrame())
                    Console.ProcessInput(bind.Command);
            }
        ...
        private void OnGUI()
        {
            ...
            {
                // TOGGLES THE WINDOW
                if ((Event.current.modifiers == EventModifiers.Control || Event.current.modifiers == EventModifiers.Command) && SceneManager.GetSceneByName("LoadScene").isLoaded)
                {
                    ToggleWindow();
                }
            }
            ...
            if (showWindow)
            {
                ...
                GUI.Window(1234567892, new Rect(-20, -20, Screen.width + 20, Screen.height + 20), new Action<int>(i => { }), string.Empty, window);
                GUI.BringWindowToBack(1234567892);
                GUI.backgroundColor = Color.white;

                GUI.Window(1234567890, windowRect, new Action<int>(DrawWindow), string.Empty, window);
                ...
                if (autoComplete)
                {
                    ...
                    GUI.Window(1234567891, completeRect, new Action<int>(DrawACWindow), acTitle, window);
            ...
            {
                if (SRSingleton<GameContext>.Instance != null)
                    SRSingleton<GameContext>.Instance.InputDirector.enabled = true;
            }
        ...
        private void DrawWindow(int id)
        {
            ...
            if (GUI.Button(ulRect, openUnityLog))
                System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo { FileName = Console.unityLogFile, UseShellExecute = true });

            if (GUI.Button(slRect, openMelonLoaderLog))
                System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo { FileName = Console.melonLoaderLogFile, UseShellExecute = true });
            ...
            if (autoAC != oldAutoAC)
            {
                if (autoAC)
                {
                    MelonLogger.Msg("Auto complete will now open automatically.");
                }
                else
                {
                    MelonLogger.Msg("Auto complete will only open by pressing CTRL+Space or CMD+Space");
                }
            ...
            if (moveCursor)
            {
                TextEditor txt = GUIUtility.GetStateObject(Il2CppType.Of<TextEditor>(), GUIUtility.keyboardControl).Cast<TextEditor>();
